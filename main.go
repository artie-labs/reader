...
func buildSource(cfg *config.Settings, statsD mtr.Client) (sources.Source, bool, error) {
	var source sources.Source
	var isStreamingMode bool
	var err error
	switch cfg.Source {
	case config.SourceDynamo:
		source, isStreamingMode, err = dynamodb.Load(*cfg.DynamoDB, statsD)
...
func main() {
	...
	source, isStreamingMode, err := buildSource(cfg, statsD)
	if err != nil {
