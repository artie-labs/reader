...
func TestGetTimeDuration(t *testing.T) {
	// Test with time at midnight
	{
		...
		assert.Equal(t, int32(0), getTimeDuration(timeVal, time.Millisecond))
	}

	// Test with time including only hours
	{
		timeVal := time.Date(2021, 1, 1, 3, 0, 0, 0, time.UTC)
		assert.Equal(t, int32(10800000), getTimeDuration(timeVal, time.Millisecond))
	...
	// Test with time including hours and minutes
	{
		...
		assert.Equal(t, int32(5400000), getTimeDuration(timeVal, time.Millisecond))
	...
	// Test with time including hours, minutes, and seconds
	{
		...
		assert.Equal(t, int32(9930000), getTimeDuration(timeVal, time.Millisecond))
	...
	// Test with time including hours, minutes, seconds, and milliseconds
	{
		...
		assert.Equal(t, int32(15615000), getTimeDuration(timeVal, time.Millisecond))
