...
func (s *S3Client) StreamJsonGzipFile(file S3File, ch chan<- dynamodb.ItemResponse) error {
	...
	if err != nil {
		return fmt.Errorf("failed to get object from S3, err: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to create a GZIP reader for object, err: %w", err)
	}
	defer gz.Close()
	...
	if err = scanner.Err(); err != nil {
		return fmt.Errorf("error reading from S3 object, err: %w", err)
	}
