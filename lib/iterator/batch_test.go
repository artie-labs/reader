...
func TestBatchIterator(t *testing.T) {
	...
	{
		batches, err := Collect(Batch(ForSlice([]int{}), 2))
		assert.NoError(t, err)
		assert.Empty(t, batches)
	}
	// length of items is 1
	{
		batches, err := Collect(Batch(ForSlice([]int{1}), 2))
		assert.NoError(t, err)
		assert.Equal(t, [][]int{{1}}, batches)
	}
	// n is 0
	{
		batches, err := Collect(Batch(ForSlice([]int{1, 2}), 0))
		assert.NoError(t, err)
		assert.Equal(t, [][]int{{1}, {2}}, batches)
	}
	// length of items is a multiple of n
	{
		batches, err := Collect(Batch(ForSlice([]int{1, 2, 3, 4}), 2))
		assert.NoError(t, err)
		assert.Equal(t, [][]int{{1, 2}, {3, 4}}, batches)
	}
	// length of items is not a multiple of n
	{
		batches, err := Collect(Batch(ForSlice([]int{1, 2, 3, 4, 5}), 2))
		assert.NoError(t, err)
